...
function processSpotData(csvData) {
    ...
    console.log("Max Grid Indices:", max_grid_x, max_grid_y);

    // 7. Construct the final 2D array (Grid)
    const finalGrid = Array(max_grid_y + 1).fill(null).map(() => Array(max_grid_x + 1).fill(null));
    ...
    df.forEach(row => {
        ...
        if (finalGrid[y_index] && finalGrid[y_index][x_index] === null) {
            finalGrid[y_index][x_index] = [];
        }

        if (finalGrid[y_index] && finalGrid[y_index][x_index]) {
            finalGrid[y_index][x_index].push(spot);
        }
    });
...
async function processAndDisplayData() {
    ...
    try {
        ...
        const response = await fetch('SpotPositions_SP1.csv');
        ...
        // Process the data
        const { finalGrid, df } = processSpotData(csvData);
        updateUI(finalGrid, df);
        console.log("Data processing complete.");
        console.log("Final 2D Grid Array (Row: grid_y, Column: grid_x):", finalGrid);
